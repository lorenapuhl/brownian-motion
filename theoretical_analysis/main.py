...
"""
Exercise 1: Simulate 1D Brownian motion (free diffusion) without external forces. The goal was to:
    Generate multiple trajectories using the Langevin equation
    Compare numerical results with the analytical solution (Gaussian distribution)
    Verify that the Mean Square Displacement (MSD) grows linearly with time (MSD ∝ t)

Exercise 2 (incomplete in your code): Simulate Brownian motion in a harmonic potential. The goal was to:
    Study confined diffusion with a restoring force
    Show that MSD saturates to a constant value at long times
    Compare with analytical predictions for the equilibrium distribution
